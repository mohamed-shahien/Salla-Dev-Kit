...
Mandatory instructions for any AI agent or developer working in this Salla theme project.

## Project Identity

- This project is a **Salla theme (Twilight) target project**.
- Salla-Dev-Kit is installed here as a workflow/skills layer: `.salla/`, `commands/`, `references/`, and `.claude/commands/`.
- Do **not** modify the Salla-Dev-Kit workflow files (`.salla/`, `commands/`, `references/`, `.claude/commands/`) unless the user explicitly asks.
- When running any `salla.*` command, work on **this theme's actual source files**, not on the workflow layer.
- Read `.salla/`, `commands/`, and `references/` before any execution.
...
6. `implement`: edit the correct theme source files only.
...
4. Execute its workflow using the current theme project context.
...
1. `AGENTS.md`
2. `commands/` when the user invokes a Salla-Dev-Kit command.
3. `.salla/workflow.md`
4. `.salla/templates/`
...
Identify the active theme root before editing. If a Theme Raed baseline is attached, it is commonly nested at:

```text
theme-raed-master/theme-raed-master/
```

Treat that as the Theme Raed root unless the user explicitly moves or chooses another root.

Important baseline facts (Theme Raed):
